...
function RideChat({ currentRole, bookingId, userName, driverName }: any) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-white rounded-2xl overflow-hidden border border-zinc-100">
      ...
      <div
        className="flex-1 overflow-y-auto space-y-3 px-4 py-4 bg-zinc-50"
        ...
        {messages.length > 0 &&
          messages.map((m, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`max-w-[72%] px-3.5 py-2.5 leading-relaxed text-sm shadow-sm rounded-2xl ${isMine ? "bg-zinc-950 text-white rounded-br-sm" : "bg-white border border-zinc-200 text-zinc-800 rounded-bl-sm"}`}
                >
                  <p className=" wrap-break-word">{m.text}</p>
                  <span className="text-[8px] text-zinc-200">
                ...
                <div className="flex flex-col gap-1.5">
                  {suggestions.map((s, i) => (
                    <motion.div
                      ...
                      className=" text-left text-sm text-zinc-700 bg-zinc-50 hover:bg-violet-500 hover:text-violet-700 border border-zinc-100 hover:border-violet-200 px-3 py-2 rounded-xl transition-all"
                    >
                      {s}
                    </motion.div>
                  ))}
        ...
        <div className="flex items-center gap-2 bg-zinc-100 rounded-2xl pl-3 pr-1.5 py-1.5">
          {messages.length > 0 && (
            <motion.button
              whileTap={{ scale: 0.9 }}
              onClick={() => getAISuggestions()}
              className={`shrink-0 w-8 h-8 rounded-xl flex items-center justify-center transition-all ${showAi ? "bg-violet-600 text-white" : "bg-white text-violet-500 hover:bg-violet-50 border border-zinc-200"}`}
            >
              <Sparkles size={14} />
            </motion.button>
          )}
